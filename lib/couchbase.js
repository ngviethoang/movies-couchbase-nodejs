...
let getCustomersAndRating = async (movieId, orderBy, limit, page) => {
    let n1qlQuery = `SELECT DISTINCT customers, META(customers).id AS cid, ratings.r AS rating FROM movies JOIN ratings ON KEY ratings.mid FOR movies JOIN customers ON KEYS ratings.cid WHERE META(movies).id = "${movieId}"`;
